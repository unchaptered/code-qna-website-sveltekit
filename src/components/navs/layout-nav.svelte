...
import { onMount } from "svelte";
  ...
  let UserNav = null;
  onMount(() => {
    UserNav = JSON.parse(localStorage.getItem('loggedInUser'));
  })
</script>
<!-- 64px-->
<header class="shadow-sm">
  <div class="flex items-center justify-between h-16 max-w-screen-xl px-4 mx-auto" >
    <div class="flex flex-1 w-0 lg:hidden">
      ...
      <a href={ UserNav === null ?  '/user/login' : '/user/' + UserNav._id} class="p-2 text-gray-600 bg-gray-100 rounded-full" type="a">
    ...
    <div class="flex items-center space-x-4">
      <!-- <a class="w-20 h-8 bg-gray-200 rounded-lg" href="/">Code Q&A</a> -->

      <form class="hidden mb-0 lg:flex">
        <div class="relative">
          <p class="text-ms font-bold">Code Solve</p>
          <!-- 검색창 (풀 스크린, Full Screen)-->
